...
export function Filters({ filters, onAuthorChange, onMaxPriceChange, authorUnique }: FiltersProps) {
  return (
    <div className="flex md:justify-between  justify-center max-w-lg mx-auto gap-x-10 mt-3">
      ...
      <div className="w-1/3 md:w-1/4 text-right">
        <label className="text-lg font-semibold text-gray-300">
          Max Price: <span className="text-blue-400">${filters.maxPrice}</span>
        </label>
        <input 
          type="range"  
          min="10" 
          max="26"
          step='2'
          className="w-full max-w-sm h-2 rounded-lg appearance-none cursor-pointer bg-gray-700 accent-blue-400"
